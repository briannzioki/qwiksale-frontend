...
test.describe("Service page – gallery vs API", () => {
  test("UI gallery equals API gallery (or live >= API); placeholders & optimizer checks", async ({ page }) => {
    ...
    const apiRes = await page.request.get(`/api/services/${id}`, { headers: { "cache-control": "no-store" } });
    ...
    const clean = uiUrls.filter((u) => !!u && !u.includes("/placeholder/") && !badHosts.some((h) => u.includes(h)));
