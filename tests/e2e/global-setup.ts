...
async function validateStoredState(
  ...
  try {
    ...
    if (ok) {
      console.log(
        `[global-setup] validated existing auth state: ${path.basename(statePath)}`,
      );
      return true;
    }
    return false;
...
async function loginAndSave(
  ...
  try {
    ...
    const signRes = await context.request.post(
    ...
    if (signRes.status() >= 400) {
      ...
      console.warn(
        `[global-setup] sign-in POST failed for ${email}: ${signRes.status()} ${
          detail || "(no body)"
        }`,
      );
      return false;
    ...
    console.warn(
      `[global-setup] login failed for ${email}:`,
      (e as Error)?.message,
    );
    return false;
  } finally {
...
async function seedDefaultState(
  ...
  // Default behaviour: guest / logged-out state to avoid bleed-through.
...
export default async function globalSetup(config: FullConfig) {
  ...
  const ADMIN_EMAIL =
    env("E2E_ADMIN_EMAIL") || env("E2E_SUPERADMIN_EMAIL") || "";
  const ADMIN_PASSWORD =
    env("E2E_ADMIN_PASSWORD") || env("E2E_SUPERADMIN_PASSWORD") || "";
  const USER_EMAIL = env("E2E_USER_EMAIL") || "";
  const USER_PASSWORD = env("E2E_USER_PASSWORD") || "";

  console.log("[global-setup] baseURL:", baseURL);
  console.log(
    "[global-setup] admin present:",
    !!ADMIN_EMAIL,
    "user present:",
    !!USER_EMAIL,
  );
  ...
  // By default we keep suites in logged-out mode.
  // If E2E_DEFAULT_ROLE is set, seed the default storage from that role instead.
