...
type Profile = {
  id: string; email: string | null; username: string | null; whatsapp: string | null;
  city: string | null; country: string | null; postalCode: string | null; address: string | null;
  image?: string | null;
};
...
export default function ProfileClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch("/api/me/profile", { cache: "no-store", credentials: "same-origin", signal: ctrl.signal, headers: { accept: "application/json" } });
        if (r.status === 401) {
        ...
        if (alive && u) {
          const em = u.email ?? ""; const un = u.username ?? "";
          setEmail(em); setInitialEmail(em);
          setUsername(un); setInitialUsername(un);
          setWhatsapp(u.whatsapp ?? ""); setCity(u.city ?? ""); setCountry(u.country ?? "");
          setPostalCode(u.postalCode ?? ""); setAddress(u.address ?? ""); setImage(u.image ?? null);
        }
      } catch { toast.error("Network error while loading profile."); }
  ...
  async function onSave(e: React.FormEvent) {
    ...
    if (nextEmail && nextEmail !== (initialEmail || "").toLowerCase()) {
      if (!EMAIL_RE.test(nextEmail)) { toast.error("Enter a valid email."); return; }
    ...
    if (nextUser && nextUser !== (initialUsername || "").trim()) {
      if (!USERNAME_RE.test(nextUser)) {
        toast.error("Username must be 3–24 chars (letters, numbers, ., _), no leading/trailing dot/underscore, no doubles.");
        return;
      }
    ...
    if (waRaw && !wa) { toast.error("Enter a valid Kenyan WhatsApp (07XXXXXXXX or +2547XXXXXXX)."); return; }
    ...
    try {
      ...
      if (!r.ok) { toast.error((j as any)?.error || "Failed to save profile."); return; }

      // keep local “initial” baselines in sync after success
  ...
  if (loading) {
    return <div className="card p-5"><p className="text-sm text-gray-600 dark:text-slate-400">Loading…</p></div>;
  }
  ...
  return (
    <form onSubmit={onSave} className="space-y-5" aria-busy={saving}>
      ...
      <div className="card p-5">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <div>
            ...
            {whatsapp ? <p className="mt-1 text-xs">Normalized: {normalizedWa ? <span className="text-emerald-600">+{normalizedWa}</span> : <span className="text-red-600">Invalid</span>}</p> : null}
          </div>
        </div>
      </div>
      ...
      <div className="card p-5 border border-red-200/60 dark:border-red-800/40">
        ...
        <p className="text-sm text-gray-600 dark:text-slate-400 mb-3">This will permanently delete your account and all your listings. This action cannot be undone.</p>
