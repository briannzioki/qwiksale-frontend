...
export default async function BillingPage() {
  ...
  const email = (session as any)?.user?.email as
    | string
    | undefined;
  ...
  const tier = ((me?.subscription as SubTier | null) ??
    "FREE") as SubTier;
  const until = me?.subscriptionUntil
    ? fmtDate(me.subscriptionUntil)
    : null;
    ...
    <main className="mx-auto max-w-2xl space-y-6 p-6">
      <header>
        ...
        <p className="mt-2 text-gray-600">
          Choose a plan and enter your M-Pesa number to receive an
          STK push.
        </p>
      </header>

      {/* Current plan summary */}
      <section className="rounded-2xl border border-gray-200 p-4 dark:border-slate-800">
