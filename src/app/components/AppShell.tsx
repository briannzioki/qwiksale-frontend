...
export default function AppShell({
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      try {
        track("nav_categories_open" as any);
      } catch {}
      ...
      return () => {
        ...
        try {
          track("nav_categories_close" as any);
        } catch {}
      };
    }
  }, [open, announce]);

  // Prefer sending users to the search route for category filters
  const categoryHref = useCallback(
    (value: string) => `/search?category=${encodeURIComponent(value)}`,
    []
  );
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            {!cats ? (
              <div className="text-sm text-gray-500 dark:text-slate-400 px-1">Loading categories…</div>
            ) : (
