...
type FetchedProduct = Partial<ProductFromStore> & {
  ...
  negotiable?: boolean;
  featured?: boolean;
  // flattened seller fields from schema
...
export default function ProductPage() {
  ...
  // Local fallback (if product wasn’t in the client store)
  ...
  // If store is ready and product not found, fetch directly from the API for robustness
  useEffect(() => {
    if (!ready || !id) return;
    if (product) return;
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        const j = (await r.json()) as FetchedProduct;
        if (!cancelled) setFetched(j);
      } catch (e: any) {
  ...
  // Seller info (prefer nested seller if your store includes it; else flattened fields)
  ...
  // WhatsApp deep link (only after reveal)
  const wa =
  ...
  if (!ready && !display) {
    // Initial skeleton while store is getting ready
    return (
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="rounded-xl border dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
          ...
          <div className="space-y-1 text-gray-700 dark:text-slate-200">
            <p>
              <span className="font-medium">Name:</span> {seller.name || "Private Seller"}
            </p>
            {seller.location && (
              <p>
          ...
          <div className="mt-4 text-xs text-gray-500 dark:text-slate-400">
            Safety: meet in public places, inspect items carefully, and never share
            sensitive information. Report suspicious activity via the listing page.
          </div>
        </div>
      </div>
      ...
      {revealOpen && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-md p-5 shadow-lg border dark:border-slate-800">
            ...
            {revealError && (
              <div className="mt-3 text-sm text-red-600">{revealError}</div>
            )}
