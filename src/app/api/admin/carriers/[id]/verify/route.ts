...
type Body = { verificationStatus?: string };
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  return withApiLogging(req, "/api/admin/carriers/[id]/verify", async (log: RequestLog) => {
    ...
    const verificationStatus = normalizeVerification(body.verificationStatus);
    if (!verificationStatus) {
      return badRequest("Invalid verificationStatus. Expected UNVERIFIED, PENDING, VERIFIED, or REJECTED.");
    }
    ...
    try {
      ...
      log.info({ carrierId: id, verificationStatus }, "admin_carrier_verify_ok");

      return jsonNoStore({
        ok: true,
        carrierId: String(updated?.id ?? id),
        verificationStatus: String(updated?.verificationStatus ?? verificationStatus),
      });
    } catch (e: any) {
      const msg = String(e?.message ?? e);
      log.error({ err: msg }, "admin_carrier_verify_failed");
      return jsonNoStore({ error: "Failed to update verification status." }, { status: 500 });
    }
  });
}
