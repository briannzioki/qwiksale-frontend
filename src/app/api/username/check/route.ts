...
export async function GET(req: NextRequest) {
  try {
    ...
    const meId = (session as any)?.user
    ...
    const available =
      !existing ||
      (!!meId && existing.id === meId);
    const reason: Reason = available
      ? null
      : "taken";
