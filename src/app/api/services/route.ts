...
export async function GET(req: NextRequest) {
  try {
    ...
    const subcategory = optStr(
      url.searchParams.get("subcategory")
    );

    // Ownership filters
    const mine =
      toBool(url.searchParams.get("mine")) === true;
    let sellerId =
    ...
    if (mine) {
      ...
      const uid = (session as any)?.user?.id as
        | string
        | undefined;
      if (!uid) {
        const res = jsonPrivate(
          { error: "Unauthorized" },
          { status: 401 }
        );
        attachVersion(res.headers);
    ...
    const featured = toBool(
      url.searchParams.get("featured")
    );
    const verifiedOnly = toBool(
      url.searchParams.get("verifiedOnly")
    );
    ...
    const pageSizeStr = url.searchParams.get(
      "pageSize"
    );
    const hasLimit =
    ...
    // -------------------- non-search filters --------------------
    const statusParam = optStr(
      url.searchParams.get("status")
    );
    const whereBase: Record<string, any> = {};
    if (
      !statusParam ||
      statusParam.toUpperCase() === "ACTIVE"
    )
    ...
    if (!chosenWhere) {
      ...
      if (ANDExtra.length)
        candidate["AND"] = ANDExtra;
      chosenWhere = candidate;
    }

    const isSearchLike =
      !!rawQ || !!category || !!subcategory;
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      console.log(
        "[/api/services ORDER primary]",
        safe(primaryOrder)
      );
      console.log(
        "[/api/services page/pageSize]",
        page,
        pageSize,
        "cursor:",
        cursor ?? null
    ...
    const response = await softTimeout(
      async () => {
        ...
        const hasMore =
          rowsRaw.length > pageSize;
        const data = hasMore
          ? rowsRaw.slice(0, pageSize)
          : rowsRaw;
        const nextCursor =
          hasMore && data.length
