...
export async function POST(
  ...
  if (!Service) return NextResponse.json({ error: "Service model not available" }, { status: 500 });
  ...
  try {
    ...
    const prev: string[] = Array.isArray(service.gallery) ? service.gallery : [];
    const exists = !!urlFromBody && prev.includes(urlFromBody);
    const nextGallery = exists || !urlFromBody ? prev : [...prev, urlFromBody];
    const nextImage = (makeCover || !service.image) && urlFromBody ? urlFromBody : service.image;
...
export async function DELETE(
  ...
  if (!url && pid) {
    const match = prev.find((g) => (publicIdFromUrl(g) || "") === pid);
    if (match) url = match;
  }
  ...
  try {
    ...
    const nextImage = service.image === url ? (nextGallery[0] ?? null) : service.image;
