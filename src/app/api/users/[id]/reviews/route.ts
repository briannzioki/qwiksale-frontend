...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!rl.ok)
      return tooMany(
        "You’re requesting reviews too fast.",
        rl.retryAfterSec
      );
    ...
    const pageSize = Number.isFinite(
      ...
      ? Math.max(
          1,
          Math.min(50, Math.trunc(rawSize))
        )
      : 10;
    ...
    const rows: ReviewRow[] =
      await db.review.findMany(args);
    const hasMore = rows.length > pageSize;

    const sliced: ReviewRow[] = hasMore
...
export async function POST(req: NextRequest) {
  try {
    ...
    const raterId = (session?.user as any)
    ...
    if (
      ...
      return setNoStore(
        json(
          {
            error:
              "Please add a short comment for ratings of 3 or below.",
          },
          { status: 400 }
        )
      );
    ...
    // Optional: ensure listing belongs to ratee
    if (listingId) {
      ...
      if (
        listing &&
        listing.sellerId &&
        listing.sellerId !== rateeId
      ) {
        return setNoStore(
          json(
            {
              error:
                "Listing does not belong to this user.",
            },
            { status: 400 }
          )
        );
    ...
    // Prevent duplicates for same listing
    if (listingId) {
      const dup =
      ...
      // Otherwise, limit one review per 30 days
      ...
      if (recent) {
        return setNoStore(
          json(
            {
              error:
                "You can review this user again after 30 days.",
            },
            { status: 429 }
          )
        );
