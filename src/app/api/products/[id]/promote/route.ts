...
export async function POST(req: NextRequest) {
  try {
    ...
    const now = Date.now();
    const activeTierExpired = me.subscriptionUntil && new Date(me.subscriptionUntil).getTime() < now;
    const tier = (activeTierExpired ? "BASIC" : (me.subscription as "BASIC" | "GOLD" | "PLATINUM")) || "BASIC";
    ...
    console.warn("[products/:id/promote POST] error:", e);
