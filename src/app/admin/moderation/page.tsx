...
function keepQuery(base: string, current: ReturnType<typeof parseParams>, overrides: Partial<{ page: number }>) {
  ...
  if (typeof current.resolved === "boolean") sp.set("resolved", current.resolved ? "1" : "0");
...
export default async function ModerationPage({
  ...
  const { items, total, totalPages, page, unresolved } = await loadReports(parsed);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <span className="rounded-full bg-white/15 px-3 py-1 text-xs">
              Unresolved: <span className="font-semibold">{unresolved.toLocaleString()}</span>
            </span>
            <span className="rounded-full bg-white/15 px-3 py-1 text-xs">
      ...
      >
        ...
        <button className="rounded bg-[#161748] px-3 py-1 text-sm text-white" aria-label="Apply filters">
          Apply
        </button>
        <Link href="/admin/moderation" className="btn-outline px-2 py-1 text-sm" aria-label="Clear filters">
          Clear
        </Link>
      </form>
      ...
      <section className="overflow-hidden rounded-xl border bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        {items.length === 0 ? (
          ...
          <>
            ...
            >
              <Link
                href={page > 1 ? keepQuery("/admin/moderation", parsed, { page: page - 1 }) : "#"}
              ...
              <Link
                href={page < totalPages ? keepQuery("/admin/moderation", parsed, { page: page + 1 }) : "#"}
